...
import { analyticsData, mockDeals, mockContacts, mockProperties } from '@/lib/mock-data'
import {
  BarChart,
  Bar,
  XAxis,
...
export default function AnalyticsPage() {
  ...
  const conversionRate = Math.round((wonDeals.length / mockDeals.length) * 100)
  const avgDealValue = wonDeals.length ? Math.round(wonDeals.reduce((s, d) => s + d.value, 0) / wonDeals.length) : 0

  const propertyStatusData = [
    { name: 'Verfügbar', value: mockProperties.filter(p => p.status === 'available').length },
    { name: 'Reserviert', value: mockProperties.filter(p => p.status === 'reserved').length },
    { name: 'Verkauft', value: mockProperties.filter(p => p.status === 'sold').length },
  ]
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
        {[
          ...
          { label: 'Aktive Kontakte', value: mockContacts.filter(c => c.status === 'active').length.toString(), color: '#3b82f6', sub: `von ${mockContacts.length} gesamt` },
        ].map(kpi => (
          <div
            key={kpi.label}
            style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', marginBottom: '16px' }}>
        ...
        <div style={{ backgroundColor: '#111111', border: '1px solid #1e1e1e', borderRadius: '10px', padding: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={analyticsData.monthlyRevenue} barSize={28}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        ...
        <div style={{ backgroundColor: '#111111', border: '1px solid #1e1e1e', borderRadius: '10px', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {analyticsData.dealFunnel.map((item, i) => {
              const max = Math.max(...analyticsData.dealFunnel.map(d => d.count))
            ...
            {analyticsData.topProperties.map((prop, i) => (
              <div key={prop.name} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: '#444444', fontSize: '0.75rem' }}>{prop.deals} Deals · {prop.views} Anfragen</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
                  <div
                    style={{
                      width: `${(prop.views / 12) * 60}px`,
                      height: '4px',
